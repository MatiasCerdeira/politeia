...
def run_summarization(args):
    """
    Corre únicamente el summarizer de noticias.

    Lee los artículos desde output_files/articulos_completos.json,
    genera un resumen para cada uno, y guarda el resultado en
    output_files/articulos_completos_con_resumenes.json.
    """
    with open(f"{config.OUTPUT_DIR}/articulos_completos.json", "r") as f:
    ...
    for i, article in enumerate(articles):
        print(
            f"🧠 Resumiendo artículo {i+1}/{len(articles)}: {article['title'][:50]}..."
        )
        try:
            resumen = summarizer.summarize_text(article["title"], article["text"])
            article["summary"] = resumen
        except Exception as e:
            print(f"❌ Error al resumir el artículo '{article['title'][:30]}': {e}")
            article["summary"] = ""

    save_to_json(
        articles, f"{config.OUTPUT_DIR}/articulos_completos_con_resumenes.json"
    )
    print("✅ Resúmenes guardados correctamente.")
...
def run_summary_vectorization(args):
    ...
    with open(f"{config.OUTPUT_DIR}/articulos_completos_con_resumenes.json", "r") as f:
        articles = json.load(f)
...
def run_clusterization(args):
    ...
    with open(f"{config.OUTPUT_DIR}/ids.json") as f:
        ids = json.load(f)
    with open(f"{config.OUTPUT_DIR}/articulos_completos_con_resumenes.json") as f:
